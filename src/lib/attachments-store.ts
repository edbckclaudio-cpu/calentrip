...
export async function saveFromDataUrl(dataUrl: string, suggestedName?: string): Promise<{ id: string; name: string; type: string; size: number }> {
  const blob = dataUrlToBlob(dataUrl);
  const name = suggestedName || `attachment-${Date.now()}`;
